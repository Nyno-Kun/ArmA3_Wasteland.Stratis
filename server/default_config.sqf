...
A3W_bleedingTime = 120;             // Time in seconds for which to allow revive after a critical injury (minimum 10 seconds) - to disable revive, go in addons\far_revive\FAR_defines.sqf
...
A3W_artilleryRadius = 250;          // Strike radius in meters within which the shells will randomly hit
...
A3W_extDB_ConfigName = "Database";      // Name of the connection config from @extDB3\extdb3-conf.ini to be used (the one within [brackets])
...
A3W_buildingLootChances = 15;      // Chance percentage that loot will spawn at each spot in a building (0 to 100)
...
A3W_sideMissionDelay = 5*6;       // Time in seconds between Side Missions
